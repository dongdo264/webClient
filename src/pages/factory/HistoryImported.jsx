...
//   const [check, setCheck] = useState(true);
  ...
  const columns = [
    { field: "batchCode", headerName: "Mã lô hàng", width: 170 },
    {
      field: "productCode",
      ...
      },
    {
      ...
      renderCell: (params) => {
        if (params.row.quantityImported > params.row.quantitySold) {
            return (
                <>
                  <button className="productListEdit" onClick={(event) => handleSubmit(event, params.row.id)}>Chuyển về nhà máy</button>
                </>
              );
        }
        return (
